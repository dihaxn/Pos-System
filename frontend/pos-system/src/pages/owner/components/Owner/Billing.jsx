  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const mockOutlets = [
          { outletId: 1, outletName: "Main Outlet" },
          { outletId: 2, outletName: "Downtown Branch" }
        ];
        setOutlets(mockOutlets);

        if (mockOutlets.length > 0) {
          const defaultOutletId = mockOutlets[0].outletId;
          ...
          setOrders(formatOrders(mockOrders));
        }
      } catch (error) {
